#!/usr/bin/env bash
# Wrapper script for installing Puppet
# This gets executed during vagrant provisioning if 'install_puppet' == true in config.yaml for a node.
# This script attempts to use the local Puppet master to install the agent. If it can't be reached, it
# will fallback to install Puppet from the Puppet Labs repos.
#
# Installing via the PE installer script will set the puppet server setting by default, which will initiate a CSR on the master. We don't
...
if [[ -L "$env_dir" && -d "$env_dir" ]]; then
  echo "==> Unlinking ${env_dir} before installation"
  rm -f "$env_dir"
fi
