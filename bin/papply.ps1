...
.DESCRIPTION
    Runs puppet apply command using Puppet code and data mounted in c:\control.
.PARAMETER Manifest
    The name of the manifest to apply.
    Default is C:\control\manifests\site.pp
.PARAMETER Options
    Optional options to pass to puppet command (ie: --debug).
    This defaults to $null.
#>
param(
   [string]$Manifest = "C:\control\manifests\site.pp"
   ,[string]$Options = $null
)
...
$InstallArgs = @("apply", "--test", "--report", "--summarize", "--modulepath", "$BaseDir/site:$BaseDir/modules:/etc/puppet/modules", "--environmentpath", "C:\", "--environment", "control", "--detailed-exitcodes", "$Options", $Manifest)

$InstallArg = "apply --test --report --summarize --modulepath $BaseDir/site:$BaseDir/modules:/etc/puppet/modules --environmentpath C:\ --environment control --detailed-exitcodes $Options $Manifest"
